...
import com.projeto.pedidovenda.model.vo.DataValor;
import com.projeto.pedidovenda.repository.filter.PedidoFilter;
...
public class Pedidos implements Serializable {
	...
	public Map<Date, BigDecimal> valoresTotaisPorData(Integer numeroDeDias, Usuario criadoPor) {
		...
		// classe Calendar apenas add, para remover (retroagir dias) foi
		...
		/*
		 * Projections é uma fábrica para instâncias de Projection. Pode aplicar
		 * uma projeção à uma consulta, chamando o setProjection(). Não tendo a
		 * necessidade de um "agrupamento por" explícito em uma consulta por
		 * critério. Um alias pode ser atribuído de forma opcional à uma
		 * projeção, assim o valor projetado pode ser referenciado em restrições
		 * (where) ou ordenações. Metodo "ge" (maior ou igual)
		 * 
		 * select date(data_criacao) as data, sum(valor_total) as valor from
		 * pedido where data_criacao >= :dataInicial and vendedor_id =
		 * :criadoPor group by date(data_criacao)
		 */
		...
		criteria.setProjection(Projections.projectionList()
				...
				.add(Restrictions.ge("dataCriacao", dataInicial.getTime()));// getTime
																			// do
																			// Calendar
																			// retorna
																			// um
		...
		// .setResultTransformer transforma o resultado da consulta em objeto
		// (DataValor)
	...
	/**
	 * Metodo serve de base para os dados vindos do banco, tendo todos os dias
	 * em sequencia (o que pode não acontecer no banco caso exista dias sem
	 * venda por exemplo)
	 * 
	 * @param numeroDeDias
	 * @param dataInicial
	...
	private Map<Date, BigDecimal> criarMapaVazio(Integer numeroDeDias, Calendar dataInicial) {
		dataInicial = (Calendar) dataInicial.clone();// clone é usado p/ não
